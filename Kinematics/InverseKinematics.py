...
from JacobianDef import *

'''This file defines the functions for the inverse kinematics for both a 5 state definition and a 6 state definition, In addition to some joint selection code
    Running it will find the solutions for (one selected pos of) the positions of Task 2, and plot it with respect to the target
'''
...
def InverseKinematics6States(EE_state_vector):
    x,y,z,rot_x,rot_y,rot_z = EE_state_vector

    z_unit_vector = [0,0,1]
    y_unit_vector = [0,1,0]
    x_unit_vector = [1,0,0]
    R = Fixed_angles_to_rotation_matrix(rot_x, rot_y, rot_z)
    EE_z_vector = np.dot(R, z_unit_vector).flatten()
    EE_y_vector = np.dot(R, y_unit_vector).flatten()
    EE_x_vector = np.dot(R, x_unit_vector).flatten()
    print("EE Vectors")
    print(EE_x_vector, EE_y_vector, EE_z_vector) #TODO Check this with the final
    pitch = np.arcsin(np.abs(np.dot(EE_z_vector, z_unit_vector))/(np.linalg.norm(z_unit_vector)*np.linalg.norm(EE_z_vector)))
    if np.dot(EE_z_vector, z_unit_vector)[0,0] < 0:
        pitch = -pitch
    projection_pitch_xy = EE_z_vector-((np.dot(EE_z_vector,z_unit_vector))/(np.linalg.norm(z_unit_vector)**2))*z_unit_vector
    EE_yaw_rotation = np.arccos(np.dot(projection_pitch_xy,y_unit_vector)/(np.linalg.norm(projection_pitch_xy)*np.linalg.norm(y_unit_vector)))
    EE_plane_yz_normal = np.cross(EE_z_vector, projection_pitch_xy).flatten()
    EE_y_initial_vector = -np.cross(EE_z_vector, EE_plane_yz_normal).flatten()

    if np.linalg.norm(EE_y_initial_vector)==0:
        roll = 0
    else:
        inside = np.dot(EE_y_vector,EE_y_initial_vector)/(np.linalg.norm(EE_y_initial_vector)*np.linalg.norm(EE_y_vector))
        inside_scalar = inside[0,0]
        if inside_scalar < -1:
            inside_scalar = -1
        elif inside_scalar > 1:
            inside_scalar = 1
        rel_roll = np.arccos(inside_scalar)
        roll = rel_roll
    ...
    '''Since this is the only "yaw" joint rotation, can see what happens if you purely follow the rotation matrix yaw -> bad'''
    # if x == 0 and y == 0:
    #     theta_shoulder_1 = EE_yaw_rotation[0,0]
    #     theta_shoulder_2 = -EE_yaw_rotation[0,0]
    # else:
    #     theta_shoulder_1 = np.arctan2(-x,y-y_shoulder) #shoulder joint
    #     theta_shoulder_2 = np.arctan2(x,-(y-y_shoulder))
    theta_shoulder_1 = np.arctan2(-x,y-y_shoulder) #shoulder joint
    ...
    yaw_difference_one = np.arctan2(-x,y-y_shoulder)-EE_yaw_rotation[0,0]
    yaw_difference_two = np.arctan2(x,-(y-y_shoulder)) + EE_yaw_rotation[0, 0]
    print(f'Difference in yaw to position vs to orientation {yaw_difference_one} and {yaw_difference_two}')
...
if __name__ == "__main__":
    ...
    point3 = [0.0, 0.0, 0.4, 0.000, -0.785, 1.570] # Position possible, but not at that orientation due to shoulder offset ### Indirect shoulder feasible, not perfectly exact though
    ...
    q_vector_1, q_vector_2, q_vector_3, q_vector_4 = InverseKinematics6States(better_point)
    # five_state_point = [0.0,0.25, 0.2, 0, np.pi/2]
    # q_vector_1, q_vector_2, q_vector_3, q_vector_4 = InverseKinematics(five_state_point)
    print(JointFeasibilityCheck(q_vector_1))
    print(JointFeasibilityCheck(q_vector_2))
    print(JointFeasibilityCheck(q_vector_3))
    print(JointFeasibilityCheck(q_vector_4))
    plot_robot_position_and_target(q_vector_1,better_point)
    plot_robot_position_and_target(q_vector_2,better_point)
    plot_robot_position_and_target(q_vector_3,better_point)
    plot_robot_position_and_target(q_vector_4,better_point)

    chosen_q, valid = SelectJointVector([q_vector_1,q_vector_2,q_vector_3,q_vector_4],old_q=q_home)

    print(Jacobian(chosen_q))
