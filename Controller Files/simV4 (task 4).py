...
class PickAndPlaceNode(Node):
    ...
    def __init__(self):
        ...
        # object / Gripper geometry
        self._OBJECT_SIZE = 0.036    # vary as per object dimensions
        self._GRASP_MARGIN = 0.001    
        ...
        target_width = self._OBJECT_SIZE - self._GRASP_MARGIN
        ...
        self._PICK_POSE  = [-0.10, 0.12, 0.02, -np.pi/2, -1.5707]
        self._PLACE_POSE = [0.10, 0.12, 0.02, -np.pi/2, -1.5707]
        ...
        # Set max objects to 2 (Trip 1: A to B. Trip 2: B to A)
        self._MAX_OBJECTS = 2       
        self._place_stack_level = 0       
        ...
        self.get_logger().info(f'Round-Trip Node Started.')


    def _get_current_pick_pose(self):
        return [self._PICK_POSE[0], self._PICK_POSE[1], self._PICK_POSE[2], self._PICK_POSE[3], -1.5707]
    

    def _get_current_place_pose(self):
        return [self._PLACE_POSE[0], self._PLACE_POSE[1], self._PLACE_POSE[2], self._PLACE_POSE[3] , -1.5707]
    ...
    def _get_pick_approach_pose(self):
        ...
        return [
            pick_pose[0],
            pick_pose[1],
            pick_pose[2] + self._APPROACH_OFFSET,
            pick_pose[3],
            pick_pose[4]
        ]
    ...
    def timer_callback(self):
        ...
        if self._state == "APPROACH_PICK":
            ...
            if elapsed >= WAIT_TIME:
                ...
                self.get_logger().info("Object secured gently. Lifting...")
            ...
            if elapsed >= WAIT_TIME:
                ...
                if self._place_stack_level >= self._MAX_OBJECTS:
                    self.get_logger().info("Return trip complete. Object is back at start.")
                    self._state = "DONE"
                    self._just_changed_state = True
                    return
                
                self.get_logger().info("First placement complete. Going Home...")
                self._state = "RESET_LIFT"
                self._just_changed_state = True

        elif self._state == "RESET_LIFT":
            # Dummy target, IK engine will override to Home
            target_cartesian = self._get_current_place_pose()
        ...
        # If we need to go Home, bypass Cartesian IK and command joints directly
        if self._state in ["RESET_LIFT", "DONE"]:
            chosen_q = np.array(self._HOME_Q)
            valid = True
        else:
            valid = False
            pitch_iter = 0
        ...
        if self._reached_target(target_cartesian):
            if self._state == "APPROACH_PICK":
                ...
                # Swap start and end positions for the return trip!
                temp = self._PICK_POSE
                self._PICK_POSE = self._PLACE_POSE
                self._PLACE_POSE = temp
                
                self.get_logger().info("Arrived at Home Position. Starting return trip...")          
    ...
    def _reached_target(self, target_cartesian):
        ...
        # Check Joint Distance instead of Cartesian distance if moving Home
        if self._state in ["RESET_LIFT", "DONE"]:
            dist = np.linalg.norm(self._old_q - np.array(self._HOME_Q))
            return dist < 0.05
...
def InverseKinematics(EE_state_vector):
    ...
    y_shoulder = 0.0452
    theta_shoulder_1 = np.arctan2(-x,y-y_shoulder) 
    ...
    x_2R_1 = GL_shoulder_to_gripcenter-GL_shoulder_to_upper-length_wrist_to_gripcenter*np.cos(pitch) 
    x_2R_2 = -(GL_shoulder_to_gripcenter+GL_shoulder_to_upper+length_wrist_to_gripcenter*np.cos(pitch)) 
    ...
    if cos_equiv_1 > 1: cos_equiv_1 = 1
    if cos_equiv_1 < -1: cos_equiv_1 = -1

    sin_equiv_1_1 = np.sqrt(1-cos_equiv_1**2)
    theta2_1_1 = np.arctan2(sin_equiv_1_1, cos_equiv_1)
    theta_lower_1_1 = theta2_1_1 + lower_beta 
    K1_1_1 = upper_length+lower_length*np.cos(theta2_1_1) 
    ...
    theta_wrist_1_1 = pitch - theta_upper_1_1 - theta_lower_1_1  

    sin_equiv_1_2 = -np.sqrt(1-cos_equiv_1**2)
    theta2_1_2 = np.arctan2(sin_equiv_1_2, cos_equiv_1)
    theta_lower_1_2 = theta2_1_2 + lower_beta 
    ...
    theta_upper_2_1 = theta1_2_1 - upper_alpha 
    theta_wrist_2_1 = pitch - theta_upper_2_1 - theta_lower_2_1  

    sin_equiv_2_2 = -np.sqrt(1-cos_equiv_2**2)
    theta2_2_2 = np.arctan2(sin_equiv_2_2,cos_equiv_2)
    theta_lower_2_2 = theta2_2_2 + lower_beta 
    ...
    theta_upper_2_2 = theta1_2_2 - upper_alpha 
    theta_wrist_2_2 = pitch - theta_upper_2_2 - theta_lower_2_2  

    theta_gripper = roll

    q_vector_1_1 = np.array([theta_shoulder_1,theta_upper_1_1,theta_lower_1_1,theta_wrist_1_1,theta_gripper]) 
    q_vector_1_2 = np.array([theta_shoulder_1, theta_upper_1_2, theta_lower_1_2, theta_wrist_1_2, theta_gripper]) 
    q_vector_2_1 = np.array([theta_shoulder_2,theta_upper_2_1,theta_lower_2_1,theta_wrist_2_1,theta_gripper]) 
    q_vector_2_2 = np.array([theta_shoulder_2, theta_upper_2_2, theta_lower_2_2, theta_wrist_2_2, theta_gripper]) 
...
def ForwardKinematics(q):
    ...
    y_shoulder = 0.0452
    length_wrist_to_gripcenter = 0.1351  
    z_world_to_upper = 0.119             
    GL_shoulder_to_upper = 0.0306
    upper_x_length = 0.028
    upper_y_length = 0.11257
    upper_length = np.sqrt(upper_x_length**2 + upper_y_length**2)
